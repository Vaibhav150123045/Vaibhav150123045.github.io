...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<
    "softwareEngineering" | "deepLearning" | "machineLearning"
  >("softwareEngineering");
  ...
  >([
    {
      sender: "bot",
      text: "Hi! Ask me anything about Vaibhav's experience, thesis, or technical background.",
    },
  ]);
    ...
    <main className="min-h-screen bg-[#FAFAFA] text-[#1A1A1A] antialiased selection:bg-neutral-200">
      <div className="max-w-3xl mx-auto px-6 py-20 space-y-24">
        ...
        <section className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm space-y-4">
          ...
          <div className="h-48 overflow-y-auto border border-neutral-100 rounded-lg p-4 bg-neutral-50 space-y-3 text-sm">
            {chatLog.map((msg, idx) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-lg px-3 py-2 ${msg.sender === "user" ? "bg-neutral-900 text-white" : "bg-neutral-200 text-neutral-800"}`}
                >
                  {msg.text}
                </div>
              </div>
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-neutral-950 text-white text-sm rounded-lg hover:bg-neutral-800 transition-colors"
            >
              Ask
            </button>
          </form>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-8">
            {experiences.map((exp, idx) => (
              <div
                key={idx}
                className="group relative border-l border-neutral-200 pl-4 space-y-2"
              >
        ...
        <section className="space-y-6">
          ...
          {/* Tabs control row */}
          <div className="flex space-x-2 border-b border-neutral-200 pb-2 overflow-x-auto scrollbar-hide">
            <button
              onClick={() => setActiveTab("softwareEngineering")}
              className={`px-3 py-1 text-xs font-mono rounded-md transition-all whitespace-nowrap ${activeTab === "softwareEngineering" ? "bg-neutral-900 text-white" : "text-neutral-500 hover:text-black"}`}
            >
              Software Eng
            </button>
            <button
              onClick={() => setActiveTab("deepLearning")}
              className={`px-3 py-1 text-xs font-mono rounded-md transition-all whitespace-nowrap ${activeTab === "deepLearning" ? "bg-neutral-900 text-white" : "text-neutral-500 hover:text-black"}`}
            >
              Deep Learning
            </button>
            <button
              onClick={() => setActiveTab("machineLearning")}
              className={`px-3 py-1 text-xs font-mono rounded-md transition-all whitespace-nowrap ${activeTab === "machineLearning" ? "bg-neutral-900 text-white" : "text-neutral-500 hover:text-black"}`}
            >
              Machine Learning
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {projects[activeTab].map((proj, idx) => (
              <div
                key={idx}
                className="border border-neutral-200 rounded-xl p-5 bg-white hover:shadow-md transition-all flex flex-col justify-between"
              >
                ...
                <div className="flex flex-wrap gap-1 pt-4">
                  {proj.tech.map((t, tIdx) => (
                    <span
                      key={tIdx}
                      className="bg-neutral-100 text-neutral-600 font-mono text-[10px] px-2 py-0.5 rounded"
                    >
                      {t}
                    </span>
                  ))}
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3 text-sm">
            <p>
              <span className="font-medium inline-block w-28 text-neutral-500">
                Languages:
              </span>{" "}
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {hobbies.map((hobby, idx) => (
              <span
                key={idx}
                className="border border-neutral-200 px-3 py-1 rounded-full text-xs text-neutral-600 bg-white shadow-sm"
              >
                {hobby}
              </span>
            ))}
